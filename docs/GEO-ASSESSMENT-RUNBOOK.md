...
测评引擎、页面、评分、报告、管理门禁和运维命令可以随应用部署。正式题库需要依次完成人工内容复核、修订 Angoff 定标和代表性新人试测。三项门禁均为 `APPROVED` 且来源状态为 `CURRENT` 后，全局管理员或题库管理员才能发布版本。
...
6. 录入内容审核、Angoff 定标、来源复核引用和最终通过线；此时试测门禁保持待批准，仍不能发布。
7. 仅向代表性新人授予受控试测资格，并由其完成登录、答题、超时、交卷、报告与打印验收；试测不占用正式认证次数。
8. 核对试测完成记录及评审结论后，单独确认试测门禁。
9. 发布题库版本，确认首页新手路线显示可用状态。
...
`ASSESSMENT_MANAGER` 是独立的题库管理员角色。它可以执行机器校验、试测名单、人工门禁、发布、停用、题目质量、个人报告与作废考试；它不能进入内容后台，也不能管理角色、系统入口、飞书知识源、同步或全站审计。所有在职飞书员工（包括题库管理员）都可参加正式测评。

生产角色分配只可由在职全局管理员在 migration 后执行，并只使用受控渠道核验出的内部 UUID。命令拒绝停用、重复、缺失或角色不符合预期的目标；输出不包含姓名：

```bash
$OROSAGA_COMPOSE --profile operations run --rm \
  ops node apps/api/dist/cli/assign-assessment-managers.js \
  --action grant \
  --actor-id <active-admin-uuid> \
  --user-id <target-uuid-1> \
  --user-id <target-uuid-2> \
  --user-id <target-uuid-3>
```

每个变更均写入审计日志。回退到不认识该角色的旧 release 前，必须用同一命令和 `--action revoke` 将三名目标恢复为 `EMPLOYEE`，然后再切换应用版本；数据库不执行降级 migration。
...
2. “录入内容 / Angoff”填写至少 10 个字符的飞书评审文档链接、评审单号或会议纪要编号，并填写最终通过线。
3. 在“管理试测名单”中只授权代表性新人；服务端仅向名单中的在职员工提供未发布版本，且最多允许一次试测。
4. 至少一位授权员工完成试测后，使用“确认试测”记录评审引用；服务端会拒绝没有完成记录的批准请求。
5. “发布”校验三项人工门禁与审计记录，将当前版本设为 `PUBLISHED`，关闭旧周期并退役同一测评的其他发布版本。

公司页或核心工作流发布新内容后，全部未归档候选和当前题库自动进入 `REVIEW_REQUIRED`，内容人工门禁回到 `PENDING_HUMAN`。来源复核日期到期后，服务端同样暂停新建考试。进行中的考试继续使用已冻结题库完成，历史报告保持可读。

## 上线验收

至少使用一个全局管理员、一个题库管理员和两个员工测试账号完成：
...
- 员工与编辑者无法进入 `/admin/assessments`，也无法读取他人考试和报告。
- 题库管理员无法进入 `/admin`、角色、飞书、同步、系统入口或全站审计接口。
- 题库管理员和全局管理员查看个人报告、发布、停用和作废操作均产生审计记录。
