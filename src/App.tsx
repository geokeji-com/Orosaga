...
const searchEntries = [
  ...atlasItems.map((item) => ({
    ...
    href: item.id === 'company' ? '/company' : item.id === 'organization' ? '/organization' : item.id === 'systems' ? '/systems' : item.id === 'workflow' ? '/workflow' : item.id === 'voices' ? '/voices' : `#${item.id}`,
  })),
  {
    title: '新人第一周清单',
    type: '学习路线',
    description: '账号、业务、协作与第一次交付的完整检查项。',
    href: '#journey',
  },
  {
    title: 'GEO 项目从立项到复盘',
    type: '运营手册',
    description: '理解一次标准项目如何在团队内流动。',
    href: '#latest',
  },
  {
    title: '为什么叫移山',
    type: '品牌故事',
    description: '名字、信念与我们看待长期工作的方式。',
    href: '#company',
  },
]

const updates = [
  { date: '07.12', type: '研究', title: 'AI 搜索正在如何改变品牌内容的组织方式' },
  { date: '07.09', type: '复盘', title: '从一次项目交付看清高质量协作的四个节点' },
  { date: '07.05', type: '手记', title: '新同事的第一个月：问题比答案更重要' },
]

const mountainMilestones = [
  { title: '寻山者', detail: '认识公司、方向与身边的同行者' },
  { title: '攀登者', detail: '掌握系统，也完成第一次真实交付', target: 'journey' },
  { title: '同行者', detail: '在协作中找到位置，也彼此成就', target: 'atlas', scrollOffset: 190 },
  { title: '布道者', detail: '把经验讲清楚，让方法被更多人使用', target: 'latest' },
  { title: '登峰者', detail: '独立负责，也带后来者继续向前', target: 'closing' },
]
...
function HomePage() {
  ...
  const [fortune, setFortune] = useState<(typeof mountainFortunes)[number] | null>(null)
  const [weather, setWeather] = useState({ temperature: '', condition: '天气正好' })
  ...
  const filteredEntries = useMemo(() => {
    ...
    if (!keyword) return searchEntries.slice(0, 5)
    return searchEntries.filter((entry) =>
      `${entry.title}${entry.type}${entry.description}`.toLowerCase().includes(keyword),
    )
  }, [query])
  ...
  useEffect(() => {
    ...
    const endpoint = 'https://api.open-meteo.com/v1/forecast?latitude=34.3416&longitude=108.9398&current=temperature_2m,weather_code&timezone=Asia%2FShanghai'
    ...
    fetch(endpoint, { signal: controller.signal })
      ...
      .catch((error: unknown) => {
        ...
        setWeather({ temperature: '', condition: '天气正好' })
      })
  ...
  const drawFortune = () => {
    const next = mountainFortunes[Math.floor(Math.random() * mountainFortunes.length)]
    setFortune(next)
  }
    ...
    <div className="site-shell">
      <header className="topbar">
        ...
        <nav className={menuOpen ? 'main-nav is-open' : 'main-nav'} aria-label="主导航">
          <a href="#atlas" onClick={() => setMenuOpen(false)}>知识地图</a>
          <a href="#journey" onClick={() => setMenuOpen(false)}>新手村</a>
          <a href="/workflow" onClick={() => setMenuOpen(false)}>工作流</a>
          <a href="/voices" onClick={() => setMenuOpen(false)}>研究与分享</a>
        </nav>

        <div className="header-actions">
          <button className="search-trigger" type="button" aria-label="搜索山海经" onClick={() => setSearchOpen(true)}>
      ...
      <main id="top">
        <section className="hero" aria-labelledby="hero-title">
          ...
          <div className="hero-inner">
            <div className="hero-copy">
              <div className="hero-kicker"><Compass size={15} /> 移山科技内部知识站</div>
              <h1 id="hero-title">
                <span>Orosaga</span>
                山海经
              </h1>
              <p>一张持续生长的公司知识地图。认识我们为何出发，也找到你接下来要走的路。</p>
            ...
            <aside className="hero-wayfinder" aria-label="今日登山指南">
              <div className="hero-today-meta">
                ...
                <span><CloudSun size={14} />西安 · {weather.temperature}{weather.condition}</span>
              </div>
              ...
              <div className="hero-coordinate">
                ...
                <button type="button" className="fortune-draw" onClick={drawFortune}>
                  <Sparkles size={15} /> 抽今日山签
                </button>
              </div>

              <div className={fortune ? 'fortune-result has-result' : 'fortune-result'} aria-live="polite">
                <span>今日山签</span>
                {fortune ? (
              ...
              <ol className="mountain-route" aria-label="成长路线">
                {mountainMilestones.map((milestone, index) => (
                  <li className={index === activeMilestone ? 'is-active' : ''} key={milestone.title}>
                    {milestone.target ? (
                      <button type="button" aria-pressed={index === activeMilestone} onClick={() => selectMilestone(index)}>
        ...
        <section className="atlas section-wrap" id="atlas" aria-labelledby="atlas-title">
          <div className="section-heading split-heading">
            <div>
              <span className="eyebrow">Knowledge atlas · 知识地图</span>
              <h2 id="atlas-title">先看见全貌，再选择一条路</h2>
            </div>
            <p>我们把散落在不同系统、文档和同事经验里的知识，重新整理成六座可以探索的山。</p>
          </div>
            ...
            {atlasItems.map((item, index) => {
              ...
              return (
                <a
                  ...
                  id={item.id}
                  href={item.id === 'company' ? '/company' : item.id === 'organization' ? '/organization' : item.id === 'systems' ? '/systems' : item.id === 'village' ? '#journey' : item.id === 'workflow' ? '/workflow' : item.id === 'voices' ? '/voices' : '#latest'}
                  key={item.id}
                >
                  <div className="atlas-card-top">
                    <span className="atlas-icon"><Icon size={21} strokeWidth={1.7} /></span>
        ...
        <section className="journey-band" id="journey" aria-labelledby="journey-title">
          <div className="section-wrap journey-inner">
            ...
            <ol className="trail">
              <li>
                ...
                <div className="trail-copy"><small>Day 1</small><strong>先安顿下来</strong><p>认识公司、团队与协作工具</p></div>
                <a className="trail-action" href="/company" aria-label="进入公司与业务页面"><ChevronRight size={19} /></a>
              </li>
              <li>
                <span className="trail-number">02</span>
                <div className="trail-copy"><small>Week 1</small><strong>读懂业务地图</strong><p>理解客户、产品和核心流程</p></div>
                <a className="trail-action" href="/workflow" aria-label="进入运营工作流页面"><ChevronRight size={19} /></a>
              </li>
              <li>
                <span className="trail-number">03</span>
                <div className="trail-copy"><small>Week 2</small><strong>完成第一次同行</strong><p>跟随标准工作流参与交付</p></div>
                <a className="trail-action" href="http://localhost:3302/prototype/submission" aria-label="进入第一次同行提交模块"><ChevronRight size={19} /></a>
              </li>
              <li>
                <span className="trail-number">04</span>
                <div className="trail-copy"><small>Month 1</small><strong>留下你的路书</strong><p>复盘、分享并更新知识地图</p></div>
                <a
                  className="trail-action"
                  href="https://wanhuxian.feishu.cn/wiki/MmcRw3MgZiJwqIkTA15clNDpnvg?fromScene=spaceOverview"
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="在飞书中打开个人分享文档"
                >
        ...
        <section className="latest" id="latest" aria-labelledby="latest-title">
          <div className="section-wrap latest-inner">
            <div className="section-heading split-heading">
              ...
              <a className="text-link dark" href="/voices">查看全部内容 <ChevronRight size={16} /></a>
            </div>
        ...
        <section className="closing section-wrap" id="closing" aria-label="共建知识库">
      ...
      {searchOpen && (
        <div className="search-overlay" role="presentation" onMouseDown={(event) => {
          ...
          <div className="search-dialog" role="dialog" aria-modal="true" aria-label="搜索山海经">
            <div className="search-field">
              ...
              <input
                ...
                aria-label="搜索关键词"
              />
              <button type="button" className="icon-button" aria-label="关闭搜索" onClick={() => setSearchOpen(false)}>
                <X size={19} />
              </button>
            </div>
            <div className="search-results">
              <span className="search-caption">{query ? `与“${query}”相关` : '建议探索'}</span>
              {filteredEntries.length > 0 ? filteredEntries.map((entry) => (
                <a href={entry.href} key={entry.title} onClick={closeAndNavigate}>
                  <span className="result-icon"><FileText size={17} /></span>
                  <div><small>{entry.type}</small><strong>{entry.title}</strong><p>{entry.description}</p></div>
                  <ChevronRight size={17} />
                </a>
              )) : (
                <div className="empty-state"><CircleHelp size={22} /><strong>暂时没找到这条路</strong><p>换一个更短的关键词试试。</p></div>
              )}
            </div>
            <div className="search-footer"><span>↑ ↓ 浏览</span><span>↵ 打开</span><span>esc 关闭</span></div>
          </div>
        </div>
      )}
    </div>
...
function App() {
  ...
  if (path === '/organization') return <OrganizationPage />
  if (path === '/systems') return <SystemsPage />
  if (path === '/voices') return <CampsPage />
  return <HomePage />
}
